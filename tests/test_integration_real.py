...
class TestRealAPIIntegration:
    ...
    def teardown_class(cls) -> None:
        ...
        if cls.test_github_repo:
            try:
                ...
                pass  # Failed to cleanup test repository
    ...
    def test_label_translation_functionality(self) -> None:
    ...
    def test_github_repository_creation_and_cleanup(self) -> None:
    ...
    def test_gitlab_attachment_detection(self) -> None:
        ...
        try:
            ...
            for issue, attachments, *note in issues_with_attachments[:1]:
                for attachment_url in attachments[:1]:
                    full_url = f"{source_project.web_url}{attachment_url}"
...
"""
            ...
            # Create the issue
            created_issue = self.test_github_repo.create_issue(
                title=test_title,
                body=test_body
            )
    ...
    def test_github_milestone_operations(self) -> None:
        ...
        try:
            ...
            test_milestone = self.test_github_repo.create_milestone(
                ...
                description="Test milestone for API testing",
                state='open'
            )
    ...
    def test_github_label_operations(self) -> None:
        ...
        try:
            ...
            special_label = self.test_github_repo.create_label(
                name="priority: high",
                color="ff6b6b",
                description="Translated priority label"
            )
    ...
    def test_github_issue_comment_operations(self) -> None:
        ...
        try:
            ...
            test_issue = self.test_github_repo.create_issue(
                ...
                body="This is a test issue for comment testing"
            )
            ...
            system_comment = test_issue.create_comment(
                "**System note:** changed milestone to %1"
            )
    ...
    def test_validation_report_generation(self) -> None:
        ...
        try:
            ...
            required_stats = [
                'gitlab_issues_total', 'github_issues_total',
                'gitlab_milestones_total', 'github_milestones_total',
                'gitlab_labels_total', 'labels_translated'
            ]
        ...
        try:
            ...
            for issue in issues:
                if issue.description and ('- [ ]' in issue.description or '- [x]' in issue.description):
                    if '#' in issue.description:  # Has issue references
                        test_issue = issue
                        break
    ...
    def test_enhanced_cross_linking_functionality(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if (issue_382.description and ('- [ ] #' in issue_382.description or '- [x] #' in issue_382.description)):
                    try:
    ...
    def test_closed_issues_and_milestones_retrieval(self) -> None:
        ...
        try:
            ...
            # Verify migrator uses state='all' correctly
            migrator = GitLabToGitHubMigrator(
                gitlab_project_path=self.source_gitlab_project,
                github_repo_path=self.target_github_repo
            )
            
            # The migrator's validation method should use state='all'
            report = migrator.validate_migration() if hasattr(migrator, 'github_repo') else None
            
            # Just verify the method doesn't crash - detailed validation needs GitHub repo
    ...
    def test_task_relationship_separation_from_regular_links(self) -> None:
        ...
        try:
            ...
            test_cases = [
                ...
                {
                    'description': "Regular issue without tasks",
                    'expected_tasks': 0,
    ...
    def test_comprehensive_cleanup_verification(self) -> None:
        ...
        try:
            ...
            test_issues = [issue for issue in all_issues 
                          if 'Test Issue' in issue.title or 'test' in issue.title.lower()]
            
            # List all milestones created during testing
            all_milestones = list(self.test_github_repo.get_milestones(state='all'))
            test_milestones = [milestone for milestone in all_milestones 
            ...
            test_labels = [label for label in all_labels 
                          if 'test' in label.name.lower() or 'priority:' in label.name]
