...
def pytest_runtest_makereport(item: pytest.Item, call: pytest.CallInfo) -> Generator[None, None, None]:  # type: ignore[type-arg]
    """
    ...
    if call.when == "call" and report.outcome == "passed":
        ...
        if warning_records:
            ...
            report.longrepr = (
                f"Integration test failed: {len(warning_records)} warning(s) detected:\n" +
                "\n".join(f"  - {msg}" for msg in warning_messages)
            )
        
        # Clean up the warnings for this test
        if test_nodeid in _integration_test_warnings:
            del _integration_test_warnings[test_nodeid]
